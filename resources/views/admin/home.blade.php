                    ...
                    @for ($j = 0; $j <= 7; $j++)
                        ...
                        @endphp
                            @foreach ($register as $re)
                                @if ($re->period == $i)
                                    @if ($re->week == $j)
                                        <td class="orenge sinsei">教室{{ $re->room_id }}<br><a href="/delete/{{$re->id}}" class="delete">取り消す</a>
                                        @if ($re->check == 1)
                                            バッティングしています
                                        @endif
                                        </td>
                            ...
                            @endforeach
                            @if ($box == 0)
                                <td class="orenge"><button id="select" type="button" class="super" data-toggle="modal" data-target="#testModal1" data-period="{{ $i }}" data-week="{{ $j }}"></button></td>
                            @endif
                        @endif
                    @endfor
                </tr>
